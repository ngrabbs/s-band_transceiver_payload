...
class RFM9x:
    ...
    def __init__(
        ...
        *,
        ...
        print("Setting high_power mode...")
        self.high_power = high_power
        print("Creating SPI device...")
        self._device = spidev.SPIDevice(spi, cs, baudrate=baudrate, polarity=0, phase=0)
        print("Setting up reset pin...")
        self._reset = reset
        ...
        print("Reading RegVersion (0x42)...")
        version = self._read_u8(_RH_RF95_REG_42_VERSION)
        print(f"RegVersion read: 0x{version:02X}")
        if version != 0x12:
            raise RuntimeError("Failed to find rfm9x with expected version -- check wiring")

        print("Setting sleep mode (RegOpMode)...")
        self.sleep()
        time.sleep(0.01)

        print("Enabling long range mode (LoRa)...")
        self.long_range_mode = True
        print(f"Long range mode set: {self.long_range_mode}")
        print("Verifying sleep mode and long range mode...")
        ...
        print("Clearing low frequency mode if needed...")
        if frequency > 525:
            print("Setting low_frequency_mode = 0")
            self.low_frequency_mode = 0

        print("Configuring FIFO base addresses...")
        print("Writing FIFO_TX_BASE_ADDR (0x0E) = 0x00")
        self._write_u8(_RH_RF95_REG_0E_FIFO_TX_BASE_ADDR, 0x00)
        print("Writing FIFO_RX_BASE_ADDR (0x0F) = 0x00")
        ...
        print(f"Setting preamble length to {preamble_length}...")
        self.preamble_length = preamble_length

        print("Setting signal bandwidth to 125 kHz...")
        self.signal_bandwidth = 200000

        print("Setting coding rate to 5...")
        self.coding_rate = 5

        print("Setting spreading factor to 7...")
        self.spreading_factor = 7

        print(f"Enabling CRC: {crc}")
        self.enable_crc = crc
        """CRC Enable state"""

        print(f"Setting AGC to {agc}")
        self.auto_agc = agc
        """Automatic Gain Control state"""

        print("Setting transmit power to 13 dBm...")
        self.tx_power = 13
    ...
    def _read_u8(self, address: int) -> int:
        ...
        val = self._BUFFER[0]
        print(f"[PY RFM9x DEBUG] SPI READ  tx: {address & 0x7F:02X} | rx: {val:02X}")
        return val
    ...
    def _write_from(self, address: int, buf: ReadableBuffer, length: Optional[int] = None) -> None:
        ...
        # buffer is written.
        tx_bytes = " ".join(f"{b:02X}" for b in buf)
        print(f"[PY RFM9x DEBUG] SPI WRITEF tx: {(address | 0x80) & 0xFF:02X} {tx_bytes}")
        print(f"[PY RFM9x DEBUG] SPI WRITEF addr: 0x{address:02X} data: 0x{tx_bytes:02X}")
        if length is None:
            length = len(buf)
        with self._device as device:
    ...
    def _write_u8(self, address: int, val: int) -> None:
        ...
        print(f"[PY RFM9x DEBUG] SPI WRITE tx: {address | 0x80:02X} {val:02X}")
        print(f"[PY RFM9x DEBUG] SPI WRITE addr: 0x{address:02X} data: 0x{val:02X}")
        with self._device as device:
